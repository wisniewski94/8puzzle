...
class AStarSolver {
    ...
    while (this.openList.length > 0) {
      ...
      if (!foundSolution) {
        ...
        const withoutCurrent = this.openList.filter((node) => {
          ...
          for (let i = 0; i < node.node.board.length; i += 1) {
  ...
  findSteps() {
    const { solution } = this;
    const steps = [];
    const reversedSolution = solution.reverse();
    for (let i = 0; i < reversedSolution.length - 1; i += 1) {
      const diff = getArrayDiff(reversedSolution[i], reversedSolution[i + 1]);
      steps.push(diff);
    }
    const stepsWithoutZero = steps.map((step) => step.filter((el) => el !== 0));
    const flatSteps = stepsWithoutZero.map((step) => step[0]);
